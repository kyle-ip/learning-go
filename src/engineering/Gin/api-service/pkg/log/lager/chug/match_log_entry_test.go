...
func MatchLogEntry(entry LogEntry) types.GomegaMatcher {
...
func (m *logEntryMatcher) Match(actual interface{}) (success bool, err error) {
	actualEntry, ok := actual.(LogEntry)
	if !ok {
		return false, fmt.Errorf("MatchLogEntry must be passed a LogEntry.  Got:\n%s", format.Object(actual, 1))
	}
