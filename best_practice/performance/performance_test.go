...
// - 使用 bufio.NewWrite() 和 bufio.NewReader() 可以带来更高的 I/O 性能。
// - 在 for-loop 里的固定的正则表达式，要使用 regexp.Compile() 编译正则表达式，性能会提升两个数量级。
// - 如果需要更高性能的协议，就要考虑使用 protobuf 或 msgp 代替 JSON（序列化和反序列化中使用了反射）。
// - 在使用 Map 时使用整型的 key 会比字符串的要快。
