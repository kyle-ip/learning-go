...
// Stringable 接口，Country 和 City 分别实现其 ToString 方法。
...
// PrintStr 接收参数类型为 Stringable，因此可以传入实现了 Stringable 接口的 Country 或 City。
// 参考 io.Read 和 ioutil.ReadAll：
...
// Go 语言的编译器没有严格检查结构是否实现了某接口所有方法。
// 如果希望强制实现接口的所有方法，可以声明 _ 变量把 nil 的空指针从 Square 转成 Shape，使得如果没有实现所有的接口方法，编译器就会报错：
